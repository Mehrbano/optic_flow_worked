...
int wscreen=640;
int hscreen=480;
int gs = 10; // grid step (pixels) // keeping min
...
// regression vectors
float[] fx, fy, ft;
int fm=3*9; // length of the vectors //change in this haults the program

// regressive vector length
    ...
    if(flagflow) {
      for(int ix=0;ix<gw;ix++) {
        ...
        for(int iy=0;iy<gh;iy++) {
          ...
          float u=df*sflowx[ig]*PI;
          float v=df*sflowy[ig];

          // draw the line segments for optical flow
          float a = pow(u*u, v*v);
          if(a >= 8) { // draw only if the length >=2.0
            float r=0.5*(1.0+u/(a+0.1));
            float g=0.5*(1.0+v/(a+0.1));
            float b=0.5*(2.0-(r+g));
            
            image(img,x0+u,y0+u,5,5);
            //image (img,x0,y0);
            stroke(255*r,255*g,255*b);
            //rect(x0+10,y0+10,4,4);
            fill(r,g,b);
            point (x0+u,y0+v);
